...
class ProductController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Get existing variant IDs to exclude from unique check
        ...
        // Custom SKU uniqueness check (exclude current product's variants)
        foreach ($request->variants as $index => $variantData) {
            $sku = $variantData['SKU'];
            $existsInOtherProducts = \App\Models\ProductVariant::where('SKU', $sku)
                ->whereNotIn('id', $existingVariantIds)
                ->exists();
            
            if ($existsInOtherProducts) {
                throw \Illuminate\Validation\ValidationException::withMessages([
                    "variants.{$index}.SKU" => "The SKU '{$sku}' is already used by another product."
                ]);
            }
        }
